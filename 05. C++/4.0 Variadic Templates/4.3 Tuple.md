...
    static_assert(
            false,
            "The non-specialized 'tuple<>' should never be
            instantiated." );
};
...
Unpacking might be necessary if we want to provide all items of a tuple as an argument to a function.

## See an inconvenient example below...
...
```cpp
/// User is required to specify all necessary indexes for addressing the tuple.
...
In order to get to the general implementation, we need to somehow generate indexes `<0, 1, 2, ..., N>`.
...
So if in previous examples we were "shrinking" the parameters pack, here we "expand" it.